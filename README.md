...
<section id="live" class="section live-stream">
    ...
    <div class="entrevistas-container">
        <div class="video-wrapper">
            <video id="live-stream-player-1" class="video-js vjs-default-skin" controls preload="metadata" poster="https://via.placeholder.com/640x360?text=En+Vivo+Milennium+TVI" width="640" height="360" data-m3u8="https://app.viloud.tv/hls/channel/c8984eee3163b175a0c725860f53749d.m3u8">
                <p class="vjs-no-js">
            ...
            <div id="error-fallback-1" class="error-fallback" style="display: none; text-align: center; padding: 20px; background: #f0f0f0; border: 1px solid #ddd;">
                <p>El stream no se puede cargar. <button onclick="retryStream('live-stream-player-1')">Reintentar</button></p>
            </div>
        </div>
        <p>Transmisión en vivo del canal principal de Milenium Tvi.</p>
        
        <div class="video-wrapper">
            <video id="live-stream-player-2" class="video-js vjs-default-skin" controls preload="metadata" poster="https://via.placeholder.com/640x360?text=MTVI2+Musical" width="640" height="360" data-m3u8="https://app.viloud.tv/hls/channel/119c56a41cef4bf9b47e6d600cc70a63.m3u8">
                <p class="vjs-no-js">
                    Para ver este video, habilita JavaScript y considera actualizar a un navegador que soporte video HTML5.
                </p>
            </video>
            <div id="error-fallback-2" class="error-fallback" style="display: none; text-align: center; padding: 20px; background: #f0f0f0; border: 1px solid #ddd;">
                <p>El stream no se puede cargar. <button onclick="retryStream('live-stream-player-2')">Reintentar</button></p>
            </div>
        </div>
        <p>Disfruta de la mejor selección de música en vivo con MTVI2 Musical.</p>
        
        <div class="video-wrapper">
            <video id="live-stream-player-3" class="video-js vjs-default-skin" controls preload="metadata" poster="https://via.placeholder.com/640x360?text=MTVI3+Peliculas" width="640" height="360" data-m3u8="https://app.viloud.tv/hls/channel/fa28724c715bb373296ca57a2dcd551c.m3u8">
                <p class="vjs-no-js">
                    Para ver este video, habilita JavaScript y considera actualizar a un navegador que soporte video HTML5.
                </p>
            </video>
        ...
        <div class="video-wrapper">
            <video id="live-stream-player-4" class="video-js vjs-default-skin" controls preload="metadata" poster="https://via.placeholder.com/640x360?text=MTVI4+Series" width="640" height="360" data-m3u8="https://app.viloud.tv/hls/channel/8823313f19b20ef55dea4f3ad8a4cab7.m3u8">
                <p class="vjs-no-js">
                    Para ver este video, habilita JavaScript y considera actualizar a un navegador que soporte video HTML5.
                </p>
            </video>
            <div id="error-fallback-4" class="error-fallback" style="display: none; text-align: center; padding: 20px; background: #f0f0f0; border: 1px solid #ddd;">
                <p>El stream no se puede cargar. <button onclick="retryStream('live-stream-player-4')">Reintentar</button></p>
            </div>
        </div>
        <p>Tus series favoritas en transmisión en vivo con MTVI4 Series.</p>
    </div>
...
<!-- Scripts: Video.js + hls.js + Proxy bypass -->
<script src="https://vjs.zencdn.net/8.10.0/video.min.js"></script>
<script src="https://cdn.jsdelivr.net/npm/hls.js@latest"></script>
<script>
    // Función para proxy m3u8 (bypass CORS/hotlinking vía CDN)
    function getProxiedM3U8(originalUrl) {
        // Usa jsDelivr como proxy simple (cachea y reenvió sin CORS estricto)
        return `https://cdn.jsdelivr.net/gh/user/proxy@latest?url=${encodeURIComponent(originalUrl)}`; // Reemplaza 'user/proxy' con un repo temporal si necesitas custom
        // Alternativa rápida: Usa CORS proxy público (temporal): `https://cors-anywhere.herokuapp.com/${originalUrl}` (activa en https://cors-anywhere.herokuapp.com/)
    }
    ...
    function retryStream(playerId) {
        ...
        player.load();
        player.play();
        console.log('Reintentando:', playerId);
    }
    ...
    document.addEventListener('DOMContentLoaded', function() {
        const playersConfig = [
            ...
            { id: 'live-stream-player-3', fallback: null },
            { id: 'live-stream-player-4', fallback: 'error-fallback-4' }
        ];

        playersConfig.forEach(config => {
            const originalM3u8 = document.getElementById(config.id).getAttribute('data-m3u8');
            const proxiedM3u8 = getProxiedM3U8(originalM3u8); // Aplica proxy
            ...
            const player = videojs(config.id, {
                ...
                sources: [{
                    src: Hls.isSupported() ? proxiedM3u8 : originalM3u8, // Usa hls.js si disponible
                ...
                console.log('Player ' + config.id + ' listo con m3u8: ' + proxiedM3u8);
                
                // Integra hls.js manualmente si Video.js no lo hace
                if (Hls.isSupported() && !this.tech_.hls) {
                    const hls = new Hls({
                        xhrSetup: function(xhr, url) {
                            xhr.setRequestHeader('Referer', 'https://viloud.tv/'); // Simula referrer de Viloud
                        },
                        enableWorker: true,
                        lowLatencyMode: true
                    });
                    hls.loadSource(proxiedM3u8);
                    hls.attachMedia(this.tech_.el_);
                    hls.on(Hls.Events.ERROR, function(event, data) {
                        console.error('HLS Error en ' + config.id + ':', data);
                        if (data.fatal) {
                            if (config.fallback) {
                                document.getElementById(config.id).style.display = 'none';
                                document.getElementById(config.fallback).style.display = 'block';
                            }
                            setTimeout(() => retryStream(config.id), 3000); // Retry cada 3s
                        }
                    });
                }
                ...
                this.on('error', function(e) {
                    ...
                    console.error('Video.js Error en ' + config.id + ':', error);
                    if (error && (error.code === 4 || error.message?.includes('CORS') || error.message?.includes('network'))) {
                        if (config.fallback) {
                            document.getElementById(config.id).style.display = 'none';
                            document.getElementById(config.fallback).style.display = 'block';
                        }
                        setTimeout(() => retryStream(config.id), 3000);
                    }
                });
                
                // Chequeo de carga
                setTimeout(() => {
                    if (this.readyState() < 2 && config.fallback) {
                        document.getElementById(config.id).style.display = 'none';
                        document.getElementById(config.fallback).style.display = 'block';
                    }
                }, 5000);
                
                // Mute inicial para autoplay (opcional)
                this.muted(true);
